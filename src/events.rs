...
impl BorshSerialize for SpotMarketVaultDepositRecordEvent {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        SPOT_MARKET_VAULT_DEPOSIT_RECORD_EVENT_DISCM.serialize(writer)?;
        self.0.serialize(writer)
    }
}
        ...
        if maybe_discm != SPOT_MARKET_VAULT_DEPOSIT_RECORD_EVENT_DISCM {
            return Err(
                std::io::Error::new(
                    ...
                    format!(
                        "discm does not match. Expected: {:?}. Received: {:?}",
                        SPOT_MARKET_VAULT_DEPOSIT_RECORD_EVENT_DISCM, maybe_discm
                    ),
                ),
            );
        }
        Ok(Self(SpotMarketVaultDepositRecord::deserialize(buf)?))
    }
}
